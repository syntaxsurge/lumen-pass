...
import { parseSettlementTokenAmount, formatSettlementToken } from '@/lib/settlement-token'
import { STELLAR_NETWORK_PASSPHRASE, STELLAR_RPC_URL } from '@/lib/stellar/config'
import { getNativeAssetContractAddress, getMarketplaceContractAddress, SETTLEMENT_TOKEN_SYMBOL } from '@/lib/config'
...
function useMarketplaceClient(publicKey?: string, signTransaction?: (xdr: string) => Promise<string>) {
  return useMemo(() => {
    const contractId = getMarketplaceContractAddress()
    if (!contractId || !publicKey || !signTransaction) return null
    return new MarketplaceClient({ contractId, rpcUrl: STELLAR_RPC_URL, networkPassphrase: STELLAR_NETWORK_PASSPHRASE, publicKey, signTransaction })
...
export default function MarketplacePage() {
  ...
  const myListings = useQuery(api.marketplace.listMine, address ? { ownerAddress: address } : 'skip')
  ...
  const wallet = (require('@/hooks/use-stellar-wallet') as any).useStellarWallet()
  ...
  const onCreate = async (values: ListingForm) => {
    ...
    try {
      ...
      await createListing({ ownerAddress: address, listingId: id.toString(), price: price.toString(), txHash: hash })
  ...
  const onBuy = async (l: Doc<'marketplaceListings'>) => {
    ...
    try {
      ...
      const tx = await client.buy({ token, id: BigInt(l.listingId), buyer: address })
      const { hash } = await tx.signAndSend()
  ...
  const onCancel = async (l: Doc<'marketplaceListings'>) => {
    ...
    if (!client) return toast.error('Wallet not ready')
    try {
      const tx = await client.cancel({ id: BigInt(l.listingId), seller: address })
      const { hash } = await tx.signAndSend()
      await cancelListing({ ownerAddress: address, listingId: l.listingId, txHash: hash })
  ...
  return (
    <div className='mx-auto w-full max-w-5xl space-y-8 p-6'>
      ...
      {open && (
        ...
        >
          <div>
            <label className='text-xs text-muted-foreground'>Listing ID (uint)</label>
            <Input placeholder='e.g. 1' {...form.register('id', { required: true })} />
          </div>
          <div>
            <label className='text-xs text-muted-foreground'>Price ({SETTLEMENT_TOKEN_SYMBOL})</label>
            <Input placeholder='0.00' {...form.register('price', { required: true })} />
          </div>
      ...
      <section className='space-y-3'>
        ...
        <div className='grid gap-3 md:grid-cols-2'>
          {(listings ?? []).map(l => (
            <div key={l._id} className='rounded-xl border border-border/70 bg-background/70 p-4'>
              <p className='text-sm font-medium text-foreground'>Listing #{l.listingId}</p>
              <p className='text-xs text-muted-foreground'>Price: {formatSettlementToken(BigInt(l.price))}</p>
              <div className='mt-3 flex gap-2'>
                <Button type='button' size='sm' className='gap-2' onClick={() => onBuy(l)}>
                  <ShoppingCart className='h-4 w-4' /> Buy
                </Button>
                {myListings?.some(m => m._id === l._id) && (
                  <Button type='button' variant='outline' size='sm' className='gap-2' onClick={() => onCancel(l)}>
