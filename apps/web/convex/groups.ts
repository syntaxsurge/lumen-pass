...
async function sanitizeAdministrators(
  ...
  const totalShare = sanitized.reduce(
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.administrators) {
      const admins = await sanitizeAdministrators(
        ctx,
        owner,
        args.administrators
      )
...
export const updateSettings = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.administrators !== undefined) {
      ...
      const existingByAdmin = new Map<
        Id<'users'>,
        (typeof existingAdmins)[number]
      >()
...
export const getMembers = query({
  ...
  handler: async (ctx, { id, viewerId }) => {
    ...
    const canAccess = visibility === 'public' || isOwner || isMember
...
export const remove = mutation({
  ...
  handler: async (ctx, { groupId, ownerAddress }) => {
    ...
    await Promise.all(administrators.map(entry => ctx.db.delete(entry._id)))
...
export const join = mutation({
  ...
  handler: async (
    ctx,
    { groupId, memberAddress, txHash, hasActivePass, passExpiresAt }
  ) => {
